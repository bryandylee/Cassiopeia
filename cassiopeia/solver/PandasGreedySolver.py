...
class PandasGreedySolver(GreedySolver.GreedySolver):
    def __init__(
        self,
    ...
    def compute_mutation_frequencies(
        ...
        frequencies = {}
        subset_matrix = character_matrix.loc[samples]

        for char in subset_matrix.columns:
            state_counts = subset_matrix[char].value_counts()
            state_counts[missing_state_indicator] = state_counts.get(missing_state_indicator, 0)
            frequencies[char] = state_counts.to_dict()
